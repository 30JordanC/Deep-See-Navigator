// facts.js
// Provides educational facts based on biome, hazard, species, and environment.
// The AI assistant can call AI_fact(type) to speak one fact at random.
...
/*-------------------------------------------------------
    ...
    - Chooses a random fact from a given category.
    - Falls back to "general" if type missing.
-------------------------------------------------------*/
export function AI_fact(type) {

    let list = FACTS[type];

    if (!list) {
        list = FACTS.general; // fallback category
    }
